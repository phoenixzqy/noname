...
class GameEvent {
  ...
  trigger(name) {
    ...
    do {
      ...
      if (lib.config.compatiblemode) {
        const map2 = lib.relatedTrigger, names2 = Object.keys(map2);
        doing.addList(
              ...
              for (const trigger of list.slice()) {
                for (const name2 of names2) {
                  if (trigger.startsWith(name2)) {
                    list.addArray(map2[name2].map((i) => i + trigger.slice(name2.length)));
                  }
                }
              }
  ...
  async debugger() {
    ...
    const input = async () => {
      const result2 = await game.promises.prompt("debugger调试");
      if (result2 === false) {
        return false;
      }
      const obj = runCode(this, result2);
